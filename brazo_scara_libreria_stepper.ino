/*
 * ============================================================================
 *  BRAZO ROBOT TIPO SCARA - Version con LIBRERIA Stepper + control de velocidad
 * ============================================================================
 *  Componentes:
 ...
 *
 *  DIFERENCIA CON LA OTRA VERSION:
 *    - Se usa la libreria <Stepper.h> en lugar de mover las bobinas a mano.
 *    - La velocidad del motor a pasos se ajusta con VELOCIDAD_MOTOR (en RPM).
 * ============================================================================
 */
...
// El 28BYJ-48 con su reductora da ~2048 pasos por vuelta completa.
const int PASOS_POR_VUELTA = 2048;

// >>> CONTROL DE VELOCIDAD <<<
// Velocidad del motor a pasos en RPM (revoluciones por minuto).
// Rango recomendado para el 28BYJ-48: aprox. 1 (muy lento) a 15 (muy rapido).
// Por encima de ~15 RPM el motor puede perder pasos o no arrancar.
// Cambia SOLO este numero para que suba/baje mas lento o mas rapido.
const int VELOCIDAD_MOTOR = 10;

// IMPORTANTE: la libreria Stepper necesita el orden de pines IN1, IN3, IN2, IN4
// (los del medio van cruzados) para que el 28BYJ-48 gire correctamente.
Stepper motorPasos(PASOS_POR_VUELTA, IN1, IN3, IN2, IN4);
...
void setup() {
  ...
  // La libreria Stepper configura sola los pines IN1-IN4 como salida.
  // Solo ajustamos la velocidad del motor.
  motorPasos.setSpeed(VELOCIDAD_MOTOR);
  ...
  Serial.println(F("   (version con libreria Stepper)"));
  Serial.println(F("========================================"));
  ...
  Serial.print(F("  Velocidad motor: ")); Serial.print(VELOCIDAD_MOTOR); Serial.println(F(" RPM"));
  Serial.println(F("----------------------------------------"));
...
// ============================================================================
//  MOTOR A PASOS: control con la libreria Stepper
// ============================================================================
// Da un paso en la direccion indicada: +1 = subir (antihorario), -1 = bajar (horario).
// La libreria se encarga del temporizado segun la velocidad configurada (setSpeed).
void darPaso(int direccion) {
  motorPasos.step(direccion);
